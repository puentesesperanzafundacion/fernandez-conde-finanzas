...
import {
  ...
  type ClientStage, type Companion, type FinanceDocument as Document,
  type Movement, type NewDocumentInput, type NewExpenseInput, type NewPaymentInput,
  type PaymentStage, type RecordEntity, type Status, type TrashItem,
} from "../lib/finance";
...
const expenseSpenders = ["Oscar", "Dan"];
...
function ExpenseComposer({ saving, onClose, onSave }: { saving: boolean; onClose: () => void; onSave: (movement: NewExpenseInput) => void }) {
  const [description, setDescription] = useState(""); const [category, setCategory] = useState("Operación"); const [amount, setAmount] = useState(""); const [spentBy, setSpentBy] = useState(expenseSpenders[0]); const [date, setDate] = useState(todayInput()); const [requestId] = useState(() => crypto.randomUUID());
  return <div className="overlay"><form className="sheet compact-sheet" onSubmit={(event) => { event.preventDefault(); if (description && spentBy && Number(amount) > 0) onSave({ description, category, amount: Number(amount), spentBy, occurredAt: toDatabaseDate(date), requestId }); }}><div className="sheet-head"><div><p className="eyebrow">MOVIMIENTO</p><h2>Registrar gasto</h2></div><button type="button" className="icon-button" onClick={onClose}><X /></button></div><label>Descripción<input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Ej. Impresiones del expediente" autoFocus /></label><label>Categoría<select value={category} onChange={(event) => setCategory(event.target.value)}>{expenseCategories.map((item) => <option key={item}>{item}</option>)}</select></label><label>¿Quién realizó el gasto?<select value={spentBy} onChange={(event) => setSpentBy(event.target.value)}>{expenseSpenders.map((name) => <option key={name}>{name}</option>)}</select><small className="field-help">Puede ser distinto de quien lo registra en la aplicación.</small></label><label>Fecha del gasto<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>Importe (MXN)<div className="money-input"><span>$</span><input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" /></div></label><div className="sheet-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={saving || !description || !spentBy || Number(amount) <= 0}><WalletCards /> {saving ? "Guardando…" : "Registrar gasto"}</button></div></form></div>;
}
...
function ExpenseEditor({ movement, saving, onClose, onSave }: { movement: Movement; saving: boolean; onClose: () => void; onSave: (movement: Movement) => void }) {
  const [description, setDescription] = useState(movement.description); const [category, setCategory] = useState(expenseCategories.includes(movement.category) ? movement.category : "Otros"); const [amount, setAmount] = useState(String(movement.amount)); const [spentBy, setSpentBy] = useState(expenseSpenders.includes(movement.spentBy) ? movement.spentBy : expenseSpenders[0]); const [date, setDate] = useState(toInputDate(movement.occurredAt));
  return <div className="overlay"><form className="sheet compact-sheet" onSubmit={(event) => { event.preventDefault(); if (description && spentBy && Number(amount) > 0) onSave({ ...movement, description, category, amount: Number(amount), spentBy, occurredAt: toDatabaseDate(date) }); }}><div className="sheet-head"><div><p className="eyebrow">EDITAR GASTO</p><h2>{movement.description}</h2></div><button type="button" className="icon-button" onClick={onClose}><X /></button></div><label>Descripción<input value={description} onChange={(event) => setDescription(event.target.value)} autoFocus /></label><label>Categoría<select value={category} onChange={(event) => setCategory(event.target.value)}>{expenseCategories.map((item) => <option key={item}>{item}</option>)}</select></label><label>¿Quién realizó el gasto?<select value={spentBy} onChange={(event) => setSpentBy(event.target.value)}>{expenseSpenders.map((name) => <option key={name}>{name}</option>)}</select></label><label>Fecha del gasto<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>Importe (MXN)<div className="money-input"><span>$</span><input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></div></label><div className="sheet-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={saving || !description || !spentBy || Number(amount) <= 0}><Check /> {saving ? "Guardando…" : "Guardar cambios"}</button></div></form></div>;
}
