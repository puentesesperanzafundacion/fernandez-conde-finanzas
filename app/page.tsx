...
type Tab = "inicio" | "documentos" | "clientes" | "movimientos" | "informes" | "seguimiento" | "reparto" | "papelera";
...
export default function HomePage() {
  ...
  const comarClients = useMemo(() => clients.filter((client) => client.practiceArea === "Migratorio/COMAR"), [clients]);
  ...
  useEffect(() => {
    if (tab === "seguimiento" && comarClients.length === 0) setTab("clientes");
  }, [tab, comarClients.length]);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {tab === "seguimiento" && <ComarTrackingView clients={comarClients} steps={comarSteps} progress={clientCaseStageProgress} savingKey={stageSavingKey} onOpenClient={setClientDetail} onToggle={(clientId, stepId, completed) => { void toggleClientCaseStage(clientId, stepId, completed); }} />}
...
function titleFor(tab: Tab) { return { inicio: "Resumen", documentos: "Presupuestos", clientes: "Clientes", movimientos: "Movimientos", informes: "Informes", seguimiento: "Seguimiento COMAR", reparto: "Reparto de utilidades", papelera: "Papelera y bitácora" }[tab]; }
...
function CaseStageToggle({ completed, saving, label, onToggle, compact = false }: { completed: boolean; saving: boolean; label: string; onToggle: () => void; compact?: boolean }) {
  return <button type="button" className={`case-stage-toggle ${completed ? "completed" : "pending"} ${compact ? "compact" : ""}`} disabled={saving} onClick={onToggle} aria-pressed={completed} aria-label={`${label}: ${completed ? "completado" : "pendiente"}. Toca para cambiar.`} title={`${label}: ${completed ? "Completado" : "Pendiente"}`}>{completed ? <Check /> : <X />}{!compact && <span>{completed ? "Completado" : "Pendiente"}</span>}</button>;
}
...
function ClientDetail({ client, steps, progress, savingKey, onClose, onEdit, onToggle }: { client: Client; steps: CaseStageStep[]; progress: ClientCaseStageProgress[]; savingKey: string; onClose: () => void; onEdit: () => void; onToggle: (stepId: number, completed: boolean) => void }) {
  return <div className="overlay client-detail-overlay" role="dialog" aria-modal="true" aria-labelledby="client-detail-title"><section className="sheet client-detail-sheet"><div className="sheet-head"><div><p className="eyebrow">FICHA DEL CLIENTE</p><h2 id="client-detail-title">{client.name}</h2></div><div className="sheet-head-actions"><button className="icon-button" aria-label={`Editar ${client.name}`} onClick={onEdit}><Pencil /></button><button className="icon-button" aria-label="Cerrar ficha" onClick={onClose}><X /></button></div></div><div className="client-profile-summary"><span className="avatar large">{client.name.split(" ").slice(0, 2).map((word) => word[0]).join("")}</span><div><StagePill stage={client.stage} /><p className={`practice-area-pill ${client.practiceArea ? "classified" : "unclassified"}`}>{client.practiceArea ?? "Sin área de práctica"}</p></div></div><div className="client-profile-data"><div><span>Clave interna</span><strong>{client.internalKey || "Sin clave"}</strong></div><div><span>Correo</span><strong>{client.email || "Sin correo"}</strong></div><div><span>Teléfono</span><strong>{client.phone || "Sin teléfono"}</strong></div></div>{client.companions.length > 0 && <div className="client-detail-companions"><strong>Acompañantes</strong>{client.companions.map((companion, index) => <span key={`${companion.name}-${index}`}>{companion.name}<small>{companion.relationship}</small></span>)}</div>}{client.practiceArea === "Migratorio/COMAR" && <ComarChecklist client={client} steps={steps} progress={progress} savingKey={savingKey} onToggle={onToggle} />}{!client.practiceArea && <div className="client-area-empty"><ClipboardCheck /><span><strong>Seguimiento sin configurar</strong><small>Edita la ficha y selecciona un área de práctica.</small></span></div>}</section></div>;
}

function ComarTrackingView({ clients, steps, progress, savingKey, onOpenClient, onToggle }: { clients: Client[]; steps: CaseStageStep[]; progress: ClientCaseStageProgress[]; savingKey: string; onOpenClient: (client: Client) => void; onToggle: (clientId: number, stepId: number, completed: boolean) => void }) {
  const groups = [...new Set(steps.map((step) => step.groupName))];
  const [selectedStepId, setSelectedStepId] = useState<number | null>(null);
  const selectedStep = steps.find((step) => step.id === selectedStepId);
  return <div className="page-content comar-tracking-page"><section className="comar-summary"><div><p className="eyebrow">CONTROL POR CASO</p><h2>Seguimiento COMAR</h2><p>Cada cambio se guarda de inmediato con el socio y la hora en la bitácora.</p></div><span className="count-pill">{clients.length} {clients.length === 1 ? "cliente" : "clientes"}</span></section><article className="panel comar-matrix-panel"><div className="comar-matrix-scroll"><table className="comar-matrix"><thead><tr><th rowSpan={2} className="client-column">Cliente</th>{groups.map((group) => <th key={group} colSpan={steps.filter((step) => step.groupName === group).length}>{group}</th>)}</tr><tr>{steps.map((step) => <th key={step.id}><button type="button" className="stage-header-button" title={`${step.order}. ${step.name}`} aria-label={`Mostrar nombre de la etapa ${step.order}: ${step.name}`} onClick={() => setSelectedStepId(step.id)}>{step.order}</button></th>)}</tr></thead><tbody>{clients.map((client) => { const completedCount = steps.filter((step) => isCaseStageCompleted(progress, client.id, step.id)).length; return <tr key={client.id}><th className="client-column"><button type="button" onClick={() => onOpenClient(client)}><strong>{client.name}</strong><small>{completedCount} de {steps.length}</small></button></th>{steps.map((step) => { const completed = isCaseStageCompleted(progress, client.id, step.id); return <td key={step.id}><CaseStageToggle compact completed={completed} saving={savingKey === `${client.id}-${step.id}`} label={`${client.name}, ${step.name}`} onToggle={() => onToggle(client.id, step.id, !completed)} /></td>; })}</tr>; })}</tbody></table></div></article><div className="comar-legend"><span><i className="completed" /> Completado</span><span><i className="pending" /> Pendiente</span><small>{selectedStep ? `${selectedStep.order}. ${selectedStep.name}` : "Toca un número para consultar el nombre completo de la etapa."}</small></div></div>;
}
