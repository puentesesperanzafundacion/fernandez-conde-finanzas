...
export default function HomePage() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {tab === "reparto" && <ProfitDistributionView settings={profitSettings} distributions={distributions} fundBalance={fundBalance} movements={movements} saving={isSaving} onSaveSettings={saveDistributionSettings} onCreateDistribution={addDistribution} />}
...
function ProfitDistributionView({ settings, distributions, fundBalance, movements, saving, onSaveSettings, onCreateDistribution }: { settings: ProfitSettings; distributions: ProfitDistribution[]; fundBalance: number; movements: Movement[]; saving: boolean; onSaveSettings: (settings: Omit<ProfitSettings, "updatedAt">) => void; onCreateDistribution: (periodStart: string, periodEnd: string) => void }) {
  ...
  return <div className="page-content distribution-page">
    <section className="fund-hero"><div><p className="eyebrow">FONDO DE LA FIRMA</p><h2>{money.format(fundBalance)}</h2><p>Saldo acumulado de los repartos guardados.</p></div><CircleDollarSign /></section>
